...
func WriteAnsi(r io.Reader, w io.Writer) error {
	ws := NewWriter(w)
	return ws.Format(r, lang.GetKeywords())
}

func (w *Writer) Format(r io.Reader, keywords lang.KeywordSet) error {
	p, err := lang.NewParser(r, keywords)
	if err != nil {
		return err
	}
	for {
...
func (w *Writer) format(stmt ast.Statement) error {
	defer w.inner.Flush()
...
func (w *Writer) formatUpdateList(stmt ast.UpdateStatement) error {
	return w.formatAssignment(stmt.List)
}

func (w *Writer) formatUpdateFrom(stmt ast.UpdateStatement) error {
...
func (w *Writer) formatUpdateWhere(stmt ast.UpdateStatement) error {
...
func (w *Writer) formatInsertValues(stmt ast.InsertStatement) error {
...
func (w *Writer) formatUpsert(stmt ast.Statement) error {
	...
	upsert, ok := stmt.(ast.UpsertStatement)
	if !ok {
...
func (w *Writer) formatSelectFrom(stmt ast.SelectStatement) error {
	return w.formatFrom(stmt.Tables)
}

func (w *Writer) formatSelectWhere(stmt ast.SelectStatement) error {
	return w.formatWhere(stmt.Where)
}

func (w *Writer) formatSelectColumns(stmt ast.SelectStatement) error {
	w.enter()
	defer w.leave()
	...
	for i, s := range stmt.Columns {
		...
		case ast.Alias:
			err = w.formatAlias(s)
		case ast.Call, ast.Binary, ast.Unary:
...
func (w *Writer) formatSelectGroupBy(stmt ast.SelectStatement) error {
...
func (w *Writer) formatSelectHaving(stmt ast.SelectStatement) error {
	w.enter()
	defer w.leave()
...
func (w *Writer) formatSelectOrderBy(stmt ast.SelectStatement) error {
...
func (w *Writer) formatSelectLimit(stmt ast.SelectStatement) error {
	if stmt.Limit == nil {
		return nil
	}
	lim, ok := stmt.Limit.(ast.Limit)
	if !ok {
...
func (w *Writer) formatReturn(stmt ast.Statement) error {
	if stmt == nil {
		return nil
	}
	w.enter()
...
func (w *Writer) formatCase(stmt ast.CaseStatement) error {
	w.writeString("CASE")
	if stmt.Cdt != nil {
		w.writeBlank()
...
func (w *Writer) formatWhen(stmt ast.WhenStatement) error {
	w.writeString(strings.Repeat(w.Indent, w.prefix))
	w.writeString("WHEN")
	w.writeBlank()
...
func (w *Writer) formatBetween(stmt ast.Between, nl bool) error {
